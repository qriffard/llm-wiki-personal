...
*you* (the LLM) build and maintain from curated sources. Purpose: Track reading (books, articles), listening (podcasts), and notes on personal viewpoints.
...
- `_index-<domain>.md` — per-domain sub-indexes for larger areas. Open only the
  ...
  `# Personal — <Domain> Index`, add a `## Pages` list of `[[page-slug]] —
...
- This wiki emphasizes these page types: entities, concepts, overview, evolving-thesis.

## Operations

### Ingest (the human gives you a source — usually a link, sometimes a file)
**Pick a mode first** (ask if it's unclear which):
- **Capture** (default — for sources you'll read and cite): archive an immutable
  local copy, then synthesize. Follow "Capture first" below.
...
**Capture mode — capture first.** Save an immutable local copy into `raw/` before
summarizing — the human should never have to download anything by hand:
- **Web article / page** → `WebFetch` it; save the extracted text as
...
blank result. **If capture fails or is partial, never assume or fabricate the
content and do not synthesize as if you have it.** Instead: set
`captured: failed — <reason>` (or `pending`) in frontmatter, record only what you
reliably know (title, link, version, how it relates), explicitly flag the gap in
the page, in `_hot.md`, and in the log, and tell the human. Any summary must be
built *only* from text you actually captured.

**Always capture the version** when the source has one — Confluence page version,
Google Doc/PDF revision, `vN` in the filename, or git SHA — into the page's
`version:` frontmatter. It's how lint detects staleness against the live source.

Then build the wiki:
1. Read the captured source. Discuss key takeaways with the human.
2. Summarize the source. For a **single source**, the concept/entity page it
   feeds can double as the summary — record the `raw/` capture in that page's
   frontmatter and cite it; don't create a near-duplicate `type: source` page.
   Create a separate `type: source` page only when one source informs many pages
   and needs its own standalone summary.
3. Update `index.md` (add the source; refresh "Recently Active"; add/adjust
   entity & concept entries) and the relevant `_index-<domain>.md` if in use.
4. Refresh `_hot.md` (create it if it doesn't exist yet): add any new active
   thread or key number; prune what's stale.
5. Update relevant entity/concept pages across `wiki/` — note where the new
   source contradicts or strengthens existing claims.
   - **Capture people.** When a source reveals who is working on what, create or
     update a person page in `wiki/people/` (role, current focus / what they're
     working on, key projects, relationships) and link it. Record relationships
     the human tells you (e.g. "X is my manager").
